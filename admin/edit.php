...
if (isset($_GET['article_id']))  {
  $article_id = trim(strip_tags($_GET['article_id']));
  try  {
    $article_data = $db->getArticle($article_id);
  }
  catch (Exception $e)  {
...
if ($_SERVER['REQUEST_METHOD'] == 'POST')  {
  ...
  try  {
    ...
    if (!$db->updateArticle($article_id, $article_title, $meta_title, $meta_desc, $summary, $content))
      throw new Exception('Unable to update article. Try again later...');
  }
  catch (Exception $e)  {
    $helper->errorHandler($e->getMessage());
  }
...
<body>
  <div id = "wrapper">
  <?php
    $html->doAdminPanelHeader('Edit article');
  ?>
  <form actiom = "" method = "post">
    <fieldset>
      <legend>Add new article:</legend>
        <label for = "title" class = "label">Article title:</label>
      	<input type = "text" name = "title" size = "35" value = "<?php echo $article_data[0]['title']; ?>"><br>
      	<label for = "meta_title" class = "label">Meta title:</label>
      	<input type = "text" name = "meta_title" size = "35" value = "<?php echo $article_data[0]['meta_title']; ?>"><br>
      	<label for = "meta_description" class = "label">Meta description:</label>
      	<input type = "text" name = "meta_description" size = "35" value = "<?php echo $article_data[0]['meta_description']; ?>"><br>
      	<label for = "summary" class = "label">Summary:</label>
      	<textarea name = "summary" cols="45" rows="5"><?php echo $article_data[0]['summary']; ?></textarea><br>
      	<label for = "content" class = "label">Content:</label>
      	<textarea name = "content" cols="45" rows="30"><?php echo $article_data[0]['content']; ?></textarea><br>
      	<input type = "submit" value = "Update" id = "add-item">
    </fieldset>
  </form>
  </div>
</body>
